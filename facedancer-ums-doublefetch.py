...
class DoubleFetchImage(DiskImage):
    ...
    def _register_sector_handler(self, sector_or_lambda, name, handler=None):
        ...
        if handler is None:
            handler = self.handle_nonspecial_sector
    ...
    def _initialize_sector_handlers(self):
        ...
        # XXX
        self._register_sector_handler(self.FIRMWARE_IMAGE_START, "Firmware Image", self.handle_firmware_read)
    ...
    def handle_mbr_read(self, address):
        ...
        # First, skip the bootstrap code and timestamp, as the
        # target device doesn't appear to be checking this.
        response  = 440 * b'\0'

        # Next, add in a disk signature. We'll just make one up, for now.
        response += b'\xDE\xAD\xBE\xEF'

        # Mark this disk as not copy protected.
        response += b'\x00\x00'

        # Add in our first partition entry, directing the target device to our
        # emulated FAT filesystem.
        response += self._generate_fat_partition_entry()

        # Add in three empty partition entries.
        response += (16 * 3) * b'\0'

        # Finally, add in the boot signature.
        response += b'\x55\xAA'

        # Return our completed sector.
        return response
    ...
    def handle_bpb_read(self, address):
        ...
        # These three bytes include the x86 instruction to jump to the bootloader.
        # This is checked by some sytems, even if it's not used. Here's a jump!
        response  = b'\xEB\x00\x90'

        # The OEM name. MSWIN4.1 seems to be the most commonly generated/used,
        # and slightly increases compatibility.
        response += b'MSWIN4.1'

        # Bytes per disk sector.
        response += self.get_sector_size().to_bytes(2, byteorder='little')

        response += b'\x01'              # sectors per cluster (1 sector = 512b clusters)
    ...
    def handle_root_dir_read(self, address):
        ...
        #response += self._generate_long_directory_entries(self.firmware_filename, b'DG10X2~1GEL')
        response = self._generate_directory_entry(b'DG10X2~1GEL', len(self.valid_firmware), 3)
    ...
    def _generate_fat_partition_entry(self):
        ...
        # This is most likely ignored in favor of relative entries.
        response += b'\x00\x00\x00'

        # Mark this partition as a FAT32 partition with CHS/LBA addressing.
        # mkfs.vfat prefers this over 0x0C.
        response += b'\x0B'

        # Indicate the end of our partition.
        # Almost definitely ignored.
        response += b'\x00\x00\x00'

        # LBA of our first sector.
        # We'll always start these at sector 2048.
        response += b'\x00\x08\x00\x00'
    ...
    def handle_fat_read(self, address):
        ...
        if address == self.FAT_START:
            ...
            response += b'\xFF\xFF\xFF\xFF' # for now, keep FW images in one extent
            return response
        else:
            return b''


    def handle_firmware_read(self, address):
        return b'HELLO WORLD'


    def handle_nonspecial_sector(self, address):
        """
    ...
    def get_partition_sectors(self):
        """ Get the amount of sectors available for use b our main FAT partition. """
    ...
    def get_sector_data(self, address):
        ...
        # Otherwise, run the unknown command handler.
            ...
            return self.handle_nonspecial_sector(address)
